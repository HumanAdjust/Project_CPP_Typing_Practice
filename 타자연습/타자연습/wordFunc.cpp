...
void WordRun(string texts[], int &tempAcc, int &tempTa, int a) 
{
	...
	for (i = 0; i < size; i++) {
		...
		if (a == 1)
			WordPrint(texts[index[i]], size, i, typo, acc, "낱말연습", num * 20 - 5, 17);
		else if (a == 2)
			WordPrint(texts[index[i]], size, i, typo, acc, "짧은 글 연습", 1, 10 + num * 3);
		else {
			if (five == 5) {
				for (int j = 0; j < 5; j++) {
		...
		while (true) {
			if (_kbhit()) { //키보드의 입력이 있다면
				...
				if (a == 1)
					...
					esc = InputKeyW(key, inlen, inword, texts[i], num, 0, 10 + num * 3, 11 + num * 3);
